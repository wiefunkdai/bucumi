...
if (($model=$_POST) && (isset($model) && is_array($model) && !empty($model))) {
    ...
    if ($model['booktitle']!=$model['booktitleold']) {
        $booktitle = $model['booktitle'];
        $booktitleold = $model['booktitleold'];
        if ($SQLConnector->query( "SELECT * FROM $tableName WHERE booktitle='$booktitle' AND booktitle!='$booktitleold';")->num_rows!=false) {
            $errors['booktitle'] = 'Judul buku telah digunakan!';
        }
    }

    if (in_array('bookcover', $requirefields)) {
        unset($requirefields[array_search('bookcover', $requirefields)]);
    }

    if (in_array('bookfilepath', $requirefields)) {
        unset($requirefields[array_search('bookfilepath', $requirefields)]);
    }
    ...
    $model['booktitle'] = ucwords(strtolower($model['booktitle']), ' ');
    ...
    if (empty($errors) && count($errors) === 0) {
        ...
        if ((empty($model['bookfilepath']) || trim($model['bookfilepath'])=='')) {
            unset($model['bookfilepath']);
        }
        if ((empty($model['bookcover']) || trim($model['bookcover'])=='')) {
            unset($model['bookcover']);
        }
        $queryFields = mysqli_query_params_builder($tableName, $model);
        $SQLQuery = $SQLConnector->query("UPDATE $tableName SET $queryFields WHERE bookid='$bookID';");    
        if ($SQLQuery) {
    ...
    $bookid = isset($_GET['id']) ? $_GET['id'] : $_POST['bookid'] ?? 'default';
    $tableName = mysqli_table_name('books');
    $SQLQuery = $SQLConnector->query( "SELECT * FROM $tableName WHERE bookid='$bookid';");
    if ($SQLQuery->num_rows === 0) {
        throw new \Exception('Data is invalid!');
    }
    $model = $SQLQuery->fetch_array(MYSQLI_ASSOC);
    $pageTitle = "Akun ".ucfirst($model['booktitle']);
    $model['booktitleold'] = $model['booktitle'];
}
?>
