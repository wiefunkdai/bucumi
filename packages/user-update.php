...
if (($model=$_POST) && (isset($model) && is_array($model) && !empty($model))) {
    ...
    if (empty($errors) && count($errors) === 0) {
        ...
        $SQLQuery = $SQLConnector->query("UPDATE $tableName SET $queryFields WHERE userid='$userID' AND userrole='admin';");    
        if ($SQLQuery) {
            @header("Location: " . create_url('user/manage'));
            exit();
        }
    }
} else {
    ...
    $SQLQuery = $SQLConnector->query( "SELECT * FROM $tableName WHERE userid='$userid' AND userrole='admin';");
    if ($SQLQuery->num_rows === 0) {
        throw new \Exception('Data is invalid!');
        exit();
    }
    $model = $SQLQuery->fetch_array(MYSQLI_ASSOC);
    $pageTitle = "Akun ".ucfirst($model['username']);
...
<?php require_once('_user.form.php'); ?>
