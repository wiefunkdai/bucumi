...
$pageTitle = "Anggota Manajemen";
$tableName = mysqli_table_name('users');
$SQLQuery = $SQLConnector->query("SELECT * FROM $tableName WHERE userrole='member';");
...
<div class="row">
    ...
    <div class="col-4 col-md-6 px-0 ps-1 d-flex gap-2 justify-content-md-end">
      <button onclick="javascript:window.open('member/create', '_self')" class="btn btn-outline-bucumi" type="button">
...
<div id="gridTable" class="grid-view grid-responsive table-responsive-sm">
  ...
  <ul class="dropdown-menu">
    <li><a class="dropdown-item" href="<?= create_url('member/update?id='.$model['userid']) ?>"><i class="bi bi-pencil-square"></i> Edit Data</a></li>
    <li><a class="dropdown-item" href="<?= create_url('member/create') ?>"><i class="bi bi-file-earmark-plus"></i> Buat Data Baru</a></li>
    <li><hr class="dropdown-divider"></li>
    <li><button type="button" class="dropdown-item" onclick="javascript:return confirm('Apakah Anda Yakin untuk Hapus Ini?')? window.open('<?= create_url('member/remove?id='.$model['userid']) ?>','_self') : false"><i class="bi bi-eraser"></i> Hapus Data</button></li>
  </ul>
</div>
                </th>
            </tr>
        ...
        <tbody>
          <?php if (count($models)===0): ?>
            ...
            <tr data-key="<?=  $model['userid'] ?? '-' ?>">
                ...
                <td><?=  $model['usernationid'] ?? '-' ?></td>
                <td><?=  $model['userbirthday'] ? date("d F Y",strtotime($model['userbirthday'])) : '-' ?></td>
                <td><?=  $model['userphone'] ?? '-' ?></td>
                <td><?=  $model['usergender']=='pria' ? 'Pria' : 'Wanita' ?></td>
