 ...
 */
?>
<form action="<?= create_url('member/'.$actionName) ?>" method="post" enctype="multipart/form-data" class="row g-3 mb-5" novalidate>
...
<?php endif; ?>
    ...
    <div class="col-md-12">
        <h4 class="mb-3">Otentikasi Anggota</h4>
    </div>
  <div class="col-md-6">
    <div class="form-floating mb-3 has-validation">
  ...
  <div class="col-md-6">
    <div class="form-floating mb-3 has-validation">
        <select id="inputStatus" name="userstatus" class="form-control form-select<?= isset($errors['userstatus']) ? ' is-invalid' : (isset($valids['userstatus']) ? ' is-valid' : '') ?>" placeholder="Status Anggota">
    ...
    <div class="d-flex align-items-center justify-content-between row">
        ...
        <div class="col-4 col-md-1 text-end">
            <?php if (isset($model['userid']) && $actionName=='update'): ?>
                <button type="button" class="btn btn-danger w-100" onclick="javascript:return confirm('Apakah Anda Yakin untuk Hapus Ini?')? window.open('<?= create_url('member/remove?id='.$model['userid']) ?>','_self') : false">Hapus</button>
            <?php endif; ?>
        </div>
    </div>
  </div>
