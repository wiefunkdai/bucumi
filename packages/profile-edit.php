...
if (($model=$_POST) && (isset($model) && is_array($model) && !empty($model))) {
    ...
    if (empty($errors) && count($errors) === 0) {
        $model['userrole'] = $authUser['userrole'];
        ...
        $SQLQuery = $SQLConnector->query("UPDATE $tableName SET $queryFields WHERE userid='$authUserID';");    
        if ($SQLQuery) {
            @header("Location: " . create_url($authUser['username']));
            exit();
        }
    }
} else {
    ...
    $authUserID = $authUser['userid'];
    $SQLQuery = $SQLConnector->query( "SELECT * FROM $tableName WHERE userid='$authUserID';");
...
<?php require_once('_profile.form.php'); ?>
